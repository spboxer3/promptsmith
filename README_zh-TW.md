...
- **輸入框整合**：使用各網站原生的 CSS Grid 或 Flex 排版機制，修正 PromptSmith 按鈕在 ChatGPT、Claude 與 Gemini 的位置。
- **響應式介面**：改善彈出視窗、設定頁、歷史紀錄與檢閱介面在不同內容及視窗狀態下的顯示。
- **互動細節**：新增清楚的焦點狀態、可取消的載入流程、剪貼簿失敗提示與更完善的對話框無障礙標示。
